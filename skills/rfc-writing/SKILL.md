...
Start with the observable lifecycle state:
...
Use the commands that match the work:

```sh
exo rfc create --title "..." --feature "..."
exo rfc edit <id> --body-file <path>
exo rfc promote <id> --stage <next-stage>
```

Create and update RFC content through `exo rfc create` and `exo rfc edit` so
the managed RFC identity, frontmatter, numbering, and projections stay
coherent. Promote only when the user explicitly asks for the stage transition
or confirms it after seeing the readiness evidence.

If Exo is absent, follow the repository's RFC process and template. Inspect the
local README, template, or prior accepted RFCs before choosing headings or
format.
